...
{
protected:
	CTimerBase(float flInitialInterval, uint64_t nTimerFlags) :
...
public:
	...
	bool IsTimerFlagSet(uint64_t iTimerFlag) { return !iTimerFlag || (m_nTimerFlags & iTimerFlag); }
	void Initialize()
	{
...
public:
	CTimer(float flInitialInterval, uint64_t nTimerFlags, std::function<float()> func, _timer_constructor_tag) :
		CTimerBase(flInitialInterval, nTimerFlags), m_func(func)
	{
	}

	static std::weak_ptr<CTimer> Create(float flInitialInterval, uint64_t nTimerFlags, std::function<float()> func);
...
void RemoveTimers(uint64_t iTimerFlag);